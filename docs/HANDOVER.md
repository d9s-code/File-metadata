...
managing RF emitter profiles: Emitters → Groups + Sources → Modes (built via a typed DSL or
...
1. **`8fe153b`** — Added a per-element `delta` (and Group `scan_delta`): a symmetric ±
...
2. **`c24d83b`** — Frontend UX pass: shared `ConfirmDialog` for destructive actions, flattened
   the Emitter editor so Modes lead (Groups/Sources demoted to a collapsible setup panel —
...
3. **`5ff651a`** — Scale/oversight pass (triggered by "an emitter may have 70+ modes"): search +
   ...
   cartesian-product runs), Group/Source scope filters + visual clustering on the ambiguity
   matrix.
...
9. **`9d3ea77`–`e05e5d9`** — While this handover doc was being written, **a concurrent session
   ...
   about deltas**), a fix making the Mode-name/Group/Source hover popovers focus-driven in
...
10. **This session** (a separate session from all of the above — picked up the repo cold via
    ...
    - Added per-Mode-Line **Range Matching**, **Frame Time** (Stagger PRI frame-time
      tolerance + engineered min/max), and Group **Ageout**. Range Matching shipped
      *twice*: the first pass built it as a single per-Mode boolean, freely toggleable;
      ...
      workflow as any other Mode Line field, not an instant toggle. Worth remembering: this user
    ...
    - Added **Emitter XML Export**: Allowed exporting a full Emitter profile (all Modes, 
      Groups, and Sources) to the standardized XML format.
    - **Resolved Alembic Migration Chaos**: Fixed a critical broken migration history 
      caused by conflicting revision lineages and dangling references. Re-engineered 
      the migration path using an idempotent, single-file consolidated migration 
      (`final_migration_fix_all.py`) that safely handles partial applications and 
      permission edge-cases. Cleaned up all temporary debugging and analysis scripts.
...
- **`EmitterEditorPage.tsx` shows Modes above the Groups/Sources setup panel** despite the
...
- **Group deletion silently cascades and deletes all its Modes** (plain confirm dialog only),
...
**Backend:**
- Zero test coverage on 6 of 12 routers before this session (`users`, `sources`, `groups`,
...
- Inconsistent delete error handling (`SourcesTable` catches `ApiRequestError` and shows it;
  `GroupsTable`/`ModesSection` deletes don't).
