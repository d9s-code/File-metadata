...
def propose_mode_draft(
    ...
    draft = Mode(
        ew_group_id=payload.ew_group_id or original.ew_group_id,
        source_id=payload.source_id or original.source_id,
        name=payload.name or original.name,
...
@router.patch("/batch", dependencies=[Depends(verify_csrf)])
def batch_update_modes(
    ew_group_id: UUID,
    payload: ModeBatchUpdate,
    db: Session = Depends(get_db),
    user=Depends(require_role(Role.editor)),
) -> dict:
    """Updates multiple modes in a single batch operation.
    Only works on non-approved modes.
    """
    mode = db.query(Mode).filter(Mode.ew_group_id == ew_group_id).first()
    if mode is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "EW Group not found")
    
    updated_count = ModeService.batch_update(
        db=db,
        ew_group_id=ew_group_id,
        payload=payload,
        user_id=user.id,
