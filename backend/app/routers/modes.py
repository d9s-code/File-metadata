...
from app.schemas.mode import ModeCreate, ModeOut, ModeUpdate, validate_pri_type_fields
...
def create_mode(
    ...
    line = ModeLine(mode_id=mode.id, **payload.line.model_dump())
    db.add(line)
    db.commit()
    db.refresh(mode)
    return mode
...
def update_mode(
    ...
    if payload.line is not None:
        ...
        for field, value in payload.line.model_dump().items():
