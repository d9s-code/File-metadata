...
export function useUpdateEwGroup(emitterId: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ ewGroupId, input }: { ewGroupId: string; input: Partial<EwGroupInput> }) =>
      ewGroupsApi.update(emitterId, ewGroupId, input),
    onSuccess: () => qc.invalidateQueries({ queryKey: ewGroupsKey(emitterId) }),
  });
}
